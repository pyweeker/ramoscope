...
print("RESOURCES TO READ : ")

print(" https://realpython.com/python-memory-management/ ")


print("https://groups.google.com/g/comp.lang.python/c/LdQIdn1DGa4 ")

print("https://nickmccullum.com/python-pointers/ ")


print("https://medium.datadriveninvestor.com/how-does-memory-allocation-work-in-python-and-other-languages-d2d8a9398543 ")

print("https://scoutapm.com/blog/python-memory-management")

print("https://stackoverflow.com/questions/9908013/weak-references-in-python")

# https://stackoverflow.com/questions/24359524/any-reason-to-create-weak-references-upon-instantiation-in-python
...
# -> IDEA : As it is not possible to select adress where to instanciate, it is possible to make plenty of instances and atfer then kill all dirty address and let survive pretty adressed objects.
...
class MyGame(arcade.Window):
    ...
    def on_draw(self):
        ...
        for obj in self.memobjs_data:  
            try:
                #arcade.draw_text(f" {obj}  size {sys.getsizeof(obj)}   dist_with_previous {obj.dist_with_previous} ", obj.pos_XY[0] + DECTEXT -self.camera.position[0], obj.pos_XY[1]-self.camera.position[1], arcade.color.GREEN, 20)
                arcade.draw_text(f" {obj}  size {sys.getsizeof(obj)}   dist_with_previous {obj.dist_with_previous:_} ", obj.pos_XY[0] + DECTEXT -self.camera.position[0], obj.pos_XY[1]-self.camera.position[1], arcade.color.GREEN, 20)
    ...
    def on_key_release(self, key, modifiers):
        ...
        if key == arcade.key.UP or key == arcade.key.W:
            ...
            refers = gc.get_referrers(baz)   # ?
            len_refers = len(gc.get_referrers(baz))  # ?

            getrefcount = sys.getrefcount(baz)

            print(f" getrefcount {getrefcount}   (+1 than expected because of object arg temp reference. )            /  refers {refers}  len_refers {len_refers} ")

            print("ctypes string_at   baz", string_at(id(baz),sys.getsizeof(baz)).hex())
            print("ctypes string_at   baz", string_at(id(baz),sys.getsizeof(baz)))


            self.memobjs_data.append(baz)

            try:
                self.memobjs_data[-1].dist_with_previous = id(self.memobjs_data[-1]) - id(self.memobjs_data[-2])
            except IndexError:
                self.memobjs_data[-1].dist_with_previous = 0
